...
- **Tabs (order):** Home (moon/stars zen scene when inactive + start/end with date picker, predicted date in subtitle + stale period detection), Calendar (bidirectional infinite-scroll calendar with period highlighting + predicted future periods), Log (avg stats + period history with swipe-to-edit/delete), Stats (5 chart visualizations using Swift Charts + custom SwiftUI)
- **Date picker sheets:** `DatePickerSheet` is a reusable sheet component used by both "Period Started" and "Period Ended" flows. Accepts label, button title, date range, and initial date. HomeViewModel's `togglePeriod()` is synchronous — it sets `showStartDatePicker` or `showEndDatePicker`, and the actual API calls happen in `startPeriod(date:)` / `endPeriod(date:)`.
- **Stale period detection:** If a period has been active for 20+ days, `HomeViewModel.checkForStalePeriod()` triggers a non-dismissible `StalePeriodSheet` on load/foreground. Suggests an end date based on average period length (fallback 5 days, capped to today). Action button is blocked until the stale period is closed.
- **Network retry:** `APIClient.request()` retries once on `-1005` (networkConnectionLost) to handle stale keep-alive connections, since URLSession doesn't auto-retry non-idempotent methods.
...
## Deployment
- **Backend:** Coolify (self-hosted) auto-deploys from `backend/` directory. Dockerfile chains `alembic upgrade head && uvicorn` so the app won't start on a broken schema. Dockerfile `HEALTHCHECK` uses Python stdlib (no curl/wget needed in slim image) — Coolify's UI healthcheck must be disabled to avoid overriding it.
